...
class Bitget:
    ...
    def get_amount(self, order_info: MarketOrder) -> float:
        if order_info.amount is not None and order_info.percent is not None and order_info.dallor is not None:
            ...
            order_info.amount = order_info.dallor *  order_info.leverage / current_price
            result = order_info.amount
    ...
    def market_entry(self, order_info: MarketOrder):
        ...
        try:
            ...
            if order_info.profit_price != None :
                param1["takeProfitPrice"] = str(order_info.profit_price)
            if order_info.stop_price != None :
                param1["stopLossPrice"] = str(order_info.stop_price)
            self.client.create_order( 
                symbol,
                order_info.type.lower(),
                order_info.side,
